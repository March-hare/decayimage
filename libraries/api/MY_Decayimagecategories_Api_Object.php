...
class Decayimagecategories_Api_Object extends Api_Object_Core {
    ...
    {
        // Find incidents
        $this->query = "SELECT category.id, parent_id, category_title, category_description, ";
        ...
        $this->query .= "LEFT JOIN `".$this->table_prefix."decayimage` ON (category.id = decayimage.category_id) ";
        $this->query .= "WHERE category_visible = 1 AND id=$id ";
    ...
    public function get_categories_by_all()
    {
        ...
        //find incidents
        $this->query = "SELECT category.id, parent_id, category_title as title, category_description as description, 
                category_color as color, category_position as position, category_image_thumb AS icon,
                decayimage.decayimage_thumb as decayimage
                FROM `".$this->table_prefix."category` 
                LEFT JOIN decayimage ON (category.id = decayimage.category_id)
                WHERE category_visible = 1 ORDER BY category_position ASC";
        ...
        {
            $item->icon = $item->icon ? $url_prefix . $item->icon : '';
            $item->decayimage = $item->decayimage ? $url_prefix . $item->decayimage : '';
