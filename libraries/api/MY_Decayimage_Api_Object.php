...
class Decayimage_Api_Object extends Api_Object_Core {
  ...
  public $default_decayimage_thumb = 'Question_icon_thumb.png';
	...
	public function perform_task()
	{
      ...
      // Get anything that has been added or changed since the date
      // TODO: there are some minor discrepencies with the return values here 
      // that I have not completely verified yet.  I think Ushahidi is adding
      // values based on a different time then the system time which may 
      // account for this, but this needs to be verified
      case "sincedate":
        if(!$this->api_service->verify_array_index($this->request, 'date')) {
          ...
          return;
        } else if (!($timestamp = strtotime($this->request['date']))) {
          $this->set_error_message(array(
            "error" => $this->api_service->get_error_msg(002)
          ));
          return;
        } else {
          $params = array('UNIX_TIMESTAMP(i.last_updated) > '. $timestamp);
          $this->response_data = $this->_get_incidents($params);
        } 
			break;
